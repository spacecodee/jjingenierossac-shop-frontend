...
@Component({
  selector: 'app-product-subcategory-autocomplete',
  imports: [HlmAutocomplete, FormsModule],
  templateUrl: './product-subcategory-autocomplete.html',
  styleUrl: './product-subcategory-autocomplete.css',
})
export class ProductSubcategoryAutocomplete implements OnInit, OnDestroy {
  ...
  loadSubcategories(name?: string): void {
    ...
    this.subcategoryService.getSubcategoriesForSelect(catId ?? undefined, name).subscribe({
      next: (response) => {
      ...
      error: (error: ApiErrorResponse) => {
        ...
        toast.error('Error al cargar subcategorías', {
          description: error.message || 'No se pudieron cargar las subcategorías de producto',
        });
      },
    });
  }
  ...
  loadAndSelectInitialSubcategory(subcategoryId: number): void {
    ...
    if (subcategory) {
      ...
      const catId = this.categoryId();
      this.subcategoryService.getSubcategoriesForSelect(catId ?? undefined).subscribe({
        next: (response) => {
